...
export function Nav() {
  ...
  const headerBg = scrolled
    ? "bg-white/80 dark:bg-[var(--color-surface)]/80 backdrop-blur-md border-b border-zinc-200/80 dark:border-zinc-800/50"
    : "";
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <a
              key={link.id}
              href={`#${link.id}`}
              className="px-3 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors rounded-md"
            >
              {link.label}
            </a>
          ))}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="md:hidden overflow-hidden border-t border-zinc-200 dark:border-zinc-800/50 bg-white dark:bg-[var(--color-surface)]"
          >
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.id}>
                  <a
                    ...
                    className="block px-3 py-2.5 text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 rounded-lg"
                  >
                    {link.label}
                  </a>
                </li>
